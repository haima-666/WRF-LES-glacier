...
class laigu:
    def __init__(self, termius = False):
        ...
        self.h4391 = readfile('laigu/laigu4391_2021-11-14to2023-10-21.csv')
        self.h4367 = readfile('laigu/laigu4367_2021-11-14to2023-10-22.csv')
        self.h4319 = readfile('laigu/laigu4319_2021-11-14 to2023-10-22 .csv')
        self.h4379 = readfile('laigu/laigu4379_2021-11-14 to2023-10-21 .csv')
        self.h4344 = readfile('laigu/laigu4344_2021-11-14to2022-06-30.csv')

        self.h4191 = readfile('laigu/laigu4191_2023-08-27 to2023-10-21 .csv')
        self.h4148 = readfile('laigu/laigu4148_2023-08-27 to2023-10-21 .csv')
        self.h4258 = readfile('laigu/laigu4258_2023-08-27 to2023-10-21 .csv')
        self.h4322 = readfile('laigu/on4322_2111-2310.csv')

        self.aw = [self.h4319, self.h4367, self.h4379, self.h4391]
        self.aw_new = [self.h4148, self.h4191, self.h4258]
...
def figure_meanT():
    ...
    glacier_total = [awspa4, aws94withoff, awslaigu]

    fig,ax = plt.subplots(2,2,figsize=(8,6))
    ax=ax.flatten()
    for hao, gll in enumerate(glacier_total):
        if hao ==0:
            color = 'green'
            shape = 's'
            label = 'Parlung 4'
        ...
        for inde, df in enumerate(gll):
            aws= df[0]
            # aws = time_filter(aws)
            flowlinedis = df[2]
            altitude = df[1]
            possible_tname = tspar4.p_tname
            for i in aws.columns:
                if i in possible_tname:
                    Tobsname = i
            ave_Tair = np.nanmean(aws[Tobsname])

            if inde ==0:
                # ax[1].errorbar(x=flowlinedis, y=ave_Tair, yerr=0.21, fmt=shape, capsize=4, ecolor='black', ms=scatter_size,
                #                mfc=color, mec='black', label=label)
                ax[0].errorbar(x=altitude, y=ave_Tair, yerr=0.5, fmt=shape, capsize=4, ecolor='black',
                               ms=scatter_size, mfc=color, mec='black', label=label)

                ax[0].legend()

            ax[1].errorbar(x= flowlinedis, y=ave_Tair, yerr=0.5,fmt=shape, capsize=4,ecolor='black', ms=scatter_size, mfc=color, mec='black')
            ax[0].errorbar(x=altitude, y=ave_Tair, yerr=0.5, fmt=shape, capsize=4, ecolor='black',
                           ms=scatter_size, mfc=color, mec='black')

            if (hao == 0 and inde==3) or (hao==0 and inde == 4) or (hao==2 and inde== 4) or (hao==1 and inde== 5):
                ax[1].errorbar(x=flowlinedis, y=ave_Tair, yerr=0.5, fmt=shape, capsize=4, ecolor='black', ms=scatter_size,
                               mfc='grey', mec='black')
                ax[0].errorbar(x=altitude, y=ave_Tair, yerr=0.5, fmt=shape, capsize=4, ecolor='black',
                               ms=scatter_size, mfc='grey', mec='black')
                offaltitude.append(altitude)
                offmeant.append(ave_Tair)
            ###大于80th百分位数的气温
            aws = aws.dropna()
            T_theshord = np.percentile(aws[Tobsname], 80)
            aws = aws[aws[Tobsname]>T_theshord]
            avehigh = np.nanmean(aws[Tobsname])


            ax[3].errorbar(x=flowlinedis, y=avehigh, yerr=0.5, fmt=shape, capsize=4, ecolor='black', ms=scatter_size,
                           mfc=color, mec='black')
            ax[2].errorbar(x=altitude, y=avehigh, yerr=0.5, fmt=shape, capsize=4, ecolor='black',
                           ms=scatter_size, mfc=color, mec='black')
            ...
            if (hao == 0 and inde == 3) or (hao == 0 and inde == 4) or (hao == 2 and inde == 4) or (hao==1 and inde== 5):
                ...
                ax[2].errorbar(x=altitude, y=avehigh, yerr=0.5, fmt=shape, capsize=4, ecolor='black',
                               ms=scatter_size, mfc='grey', mec='black')
                offmeanth.append(avehigh)
                offaltitudeh.append(altitude)

    w1, b1 = slope_cal(offaltitude, offmeant)
    w2, b2 = slope_cal(offaltitudeh, offmeanth)
    x = np.arange(4000, 5600, 100)
    ax[0].plot(x, w1*x+b1, linestyle='--', c='black')
    ax[2].plot(x, w2 * x + b2, linestyle='--', c='black')
    ax[0].set_xlim(4000, 5400)
    ax[2].set_xlim(4000, 5400)
    ax[0].grid(True, alpha=0.6)
    ax[1].grid(True, alpha=0.6)
    ax[2].grid(True, alpha=0.6)
    ax[3].grid(True, alpha=0.6)

    ax[0].set_ylabel('Mean Ta(℃)')
    ax[0].set_title('Ta All')
    ax[2].set_ylabel('Mean Ta(℃)')
    ax[2].set_title('Ta > 80th')
...
def mean_T():
    ...
    df4340 = tslaigu.h4258  #3
    df4258 = tslaigu.h4148  #1
    df4387 = readfile(r'C:\Users\admin\Desktop\code\ranwuT2m\laigu\laigu4299_1_2023-08-27 to2023-10-21 .csv')#4
    df4442 = tslaigu.h4379  #5
    df4456 = tslaigu.h4367  #6
    df4299 = tslaigu.h4191  #2
